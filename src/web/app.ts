...
class App {
  ...
  /**
   * Actionsが生成した静的JSONをフェッチする
   */
  private async loadData(prefCode: string) {
    this.container.innerHTML = '<p class="loading">データを読み込み中...</p>';

    try {
      // 取得先は Actions がデプロイした data/ フォルダ内
      ...
      this.container.innerHTML =
        '<p class="error">データの取得に失敗しました。まだデータが生成されていない可能性があります。</p>';
    }
  }

  private renderList(companies: Enterprise[]) {
